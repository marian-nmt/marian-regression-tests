...
$MRT_TOOLS/diff-floats.py $(pwd)/lm.out $(pwd)/lm.expected -p 0.02 | tee $(pwd)/lm.diff | head
...
$MRT_TOOLS/diff-floats.py $(pwd)/lm.scores.out $(pwd)/lm.scores.expected -p 0.002 | tee $(pwd)/lm.scores.diff | head
